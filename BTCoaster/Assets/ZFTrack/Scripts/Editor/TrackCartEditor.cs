	...
	protected void RenderToolbox() {
		...
		if (GUILayout.Button(allHaveTrack ? "Snap to Track" : "Find and Snap to Track")) {
			foreach (var cart in SelectedCarts) {
				...
				var localPos = cart.CurrentTrack.transform.InverseTransformPoint(cart.transform.position);
				var fraction = curve.GetFraction(localPos);
				fraction = Mathf.Max(0, Mathf.Min(fraction, 1));
				var pos = curve.GetPointAt(fraction).LocalToAbsolute(cart.CurrentTrack.transform);
